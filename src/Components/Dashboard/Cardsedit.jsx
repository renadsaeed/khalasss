...
import Donationformedit from "./Donationformedit";
  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex-grow flex flex-col" dir="rtl">
        ...
        <div className="mb-4">
          <div className="flex justify-between mb-1">
            ...
            <span className="text-sm font-medium text-gray-700">
              ${(collectedAmount ?? 0).toLocaleString()}
            </span>
          </div>
        ...
        <div className="mt-auto pt-4 border-t border-gray-100 grid grid-cols-2 gap-x-4 gap-y-3 text-sm text-gray-800">
          <div className="flex items-center gap-2">
            <FaUsers className="text-[#0D8F75] text-lg" />
            <span className="font-medium">{`${
              numberOfDonors ?? 0
            } متبرع`}</span>
          </div>
          <div className="flex items-center gap-2">
            <FaChartBar className="text-[#0D8F75] text-lg" />
            <span className="font-medium">{`${pageVisits ?? 0} زيارة`}</span>
          </div>
          <div className="flex items-center gap-2 col-span-2">
            <FaClock className="text-[#0D8F75] text-lg" />
            <span className="font-medium">
              {`آخر تبرع: ${lastDonationAgo || "لا يوجد"}`}
            </span>
          </div>
        </div>
      </div>
      <Donationformedit
        ...
        setDetailedDonation={setDetailedDonation}
